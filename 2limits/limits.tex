...
\footnotetext{%
	$N$ may be quantified as either a real or a natural number (equivalent by the Archimedean property, Theorem \ref{thm:archprop}). %: if $N\in\R$ satisfies the definition, then $\exists\widetilde N\in\N$ such that $\widetilde N\ge N$; but then $n>\widetilde N\implies n>N\ldots$ 
	It tends to be easier to use $N\in\R$ for convergence and $N\in\N$ when directly proving \emph{divergence} (see Definition \ref{defn:divergence1}).%
}
