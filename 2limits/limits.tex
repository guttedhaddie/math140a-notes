...
The original sequence $\textcolor{red}{(s_n)}$ is wedged between
...
\footnotetext{%
	$(v_N)$ being monotone-down is crucial: if $N$ satisfies $v_N-v<\frac 1{2k}$, so does $N_k:=\max(N,1+n_{k-1})$.%
}
...
\begin{exercises}{}{}
	...
	\begin{enumerate}
	  ...
	  Consider the sequences with the following $n\th$ terms:
		...
		\]
		\begin{enumerate}
		  \item For each sequence: give an example of a monotone subsequence, state the set of subsequential limits, and state the limits superior and inferior.
